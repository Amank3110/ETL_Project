...
LOGFILE="myProject_script_sample.sh_$(date +%Y%m%d%H%M%S).log"
...
STEP_NUMBER='2'
    ...
    $SNOWSQL_PATH snowsql -c "$PROJECT_CONNECTION_NAME" -f $SQL_DIR/s3ToSnowflake.sql -o exit_on_error=true -D "SF_QUERY_TAG=$SF_QUERY_TAG"
