...
LOGFILE="git_etl_script.sh_$(date +%Y%m%d%H%M%S).log"
...
STEP_NUMBER='1'
    bash $SHELL_DIR/git_script_sample.sh
...
STEP_NUMBER='3'
    ...
    $SNOWSQL_PATH snowsql -c $PROJECT_CONNECTION_NAME -f $SQL_DIR/git_Inter_table.sql -o exit_on_error=true -D "SF_QUERY_TAG=${SF_QUERY_TAG}"
...
STEP_NUMBER='5'
    SF_QUERY_TAG="insert_data_into_FINAL_TABLE_${STEP_NUMBER}"
    echo "Insert Data into FINAL_FRIENDS_DATA table $(date +%Y%m%d%H%M%S)"
    $SNOWSQL_PATH snowsql -c $PROJECT_CONNECTION_NAME -f $SQL_DIR/git_Final_table.sql -o exit_on_error=true -D "SF_QUERY_TAG=${SF_QUERY_TAG}"
